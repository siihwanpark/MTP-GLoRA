...
class DistributedModel(ABC):
    ...
    def load_checkpoint(self, checkpoint_path: str, cache_dir: Optional[str] = None):
        for ckpt_file in tqdm(self._load_ckpt_files(checkpoint_path, cache_dir), desc="Loading checkpoint"):
    ...
    def _load_ckpt_files(
        ...
        """
        Load the checkpoint files of the target model.

        Args:
            model_path (str): The path to the huggingface repository.
            cache_dir (Optional[str]): The cache directory of the model.
        """
